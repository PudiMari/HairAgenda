...
- **GET `/services/`**: Lista todos os serviços. Aceita parâmetro `professional_id` para filtragem.
- **POST `/services/`**: Cria um novo serviço (requer `name`, `price`, `duration_minutes`).
- **PATCH `/services/{id}/`**: Atualiza parcialmente um serviço existente.
- **DELETE `/services/{id}/`**: Remove um serviço do catálogo.

### Agendamentos (`/appointments/`)
Gestão do fluxo transacional de reservas.
- **GET `/appointments/`**: Lista agendamentos. Aceita `professional_id` ou `client_id` para filtros de histórico.
- **POST `/appointments/`**: Realiza uma nova reserva. 
...
- **GET `/professional-profile/{user_id}/`**: Recupera dados detalhados do perfil.
- **POST `/professional-profile/`**: Cria o perfil inicial após o onboarding.
- **PATCH `/professional-profile/{id}/`**: Atualiza dados de localização, bio e redes sociais.

### Saúde do Sistema (`/health/`)
- **GET `/health/`**: Verifica a conectividade com o banco de dados e o status da API.
